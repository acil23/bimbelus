...
export const priceUnits: Record<string, string> = {
  ...
  PER_PERIODE: "per periode",
  PER_BULAN: "per bulan",
  PER_PERTEMUAN: "per pertemuan",
...
export function whatsappUrl(value?: string | null, message?: string) {
  let number = value?.replace(/\D/g, "") ?? "";

  if (number.startsWith("0")) number = `62${number.slice(1)}`;

  if (number.length < 8 || number.length > 15) return undefined;
